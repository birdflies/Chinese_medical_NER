...
from blstm_crf.blstm_crf_layer import BLSTM_CRF
...
class NerProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, 'train_bioes_char.txt')), 'train'
        )

    def get_dev_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, 'dev_bioes_char.txt')), 'dev'
        )

    def get_test_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, "test_bioes_char.txt")), "test")
...
def file_based_convert_examples_to_features(
    ...
    for i, label in enumerate(label_list):
        label_map[label] = i
    with open('./output/label2id.pkl', 'wb+') as wf:
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):
        ...
        total_loss, logits, trans, pred_ids = create_model2(
            ...
            label_ids, num_labels, use_one_hot_embeddings=False, dropout_rate=0.5, lstm_size=128)
